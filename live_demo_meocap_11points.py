...
class meocap_imu_set:
    ...
    def get(self):
        R = torch.tensor([Rotation.from_quat(imu.rot).as_matrix() for imu in self.imus]).to(torch.float).view(11, 3, 3)
        a = torch.tensor([imu.acc for imu in self.imus]).view(-1, 3).view(11, 3)
...
async def recv_imu():
    ...
    m.start()
    frame_count = 0
    imu_names = ["左手腕", "右手腕", "左小腿", "右小腿", "头部", "腰部", "左上臂", "右上臂", "左大腿", "右大腿","胸部"]
    acc_texts = ["不可信", "低准确度", "中准确度", "高准确度"]
    while True:
        clock.tick(60)
        rpt: list[meocap_imu] = m.poll()
        if rpt is not None and len(rpt) == 10:
            rpt.append(rpt[5])
            global data_loaded
            data_loaded = True
            imu_set.imus = rpt
            # 需要在这里替换成OpenVR所得头显信息
            #imu_set.imus[4] = .....
            frame_count += 1
            if frame_count % 60 == 0:
                text = ""
...
def tpose_calibration():
    ...
    a_pose_mat = torch.transpose(torch.tensor(Rotation.from_euler("xyz", euler, degrees=True).as_matrix()).view(3, 3),
                                 0, 1)
    RMI = M_C.double().mm(a_pose_mat.t())
    RSB = torch.stack([(RMI.matmul(r.double())).t() for r in rot]).view(11, 3, 3)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        pose, tran = net.forward_frame(aM.float().view(1, 11, 3), RMB.float().view(1, 11, 3, 3))
        net.set_pose(pose,tran)
