...
#
    def calc_M(self, q):
        ...
        M = rbdl.CompositeRigidBodyAlgorithm(self.model, q, self.uk)
        return M
#
    def calc_h(self, q, qdot):
        ...
        h = rbdl.NonlinearEffects(self.model, q, qdot)

        # """ NonlinearEffects(arg0: pyrbdl.Model, arg1: numpy.ndarray[float64[m, 1]], arg2: numpy.ndarray[float64[m, 1]], arg3: List[numpy.ndarray[float64[6, 1]]]) -> numpy.ndarray[float64[m, 1]] """
        return h
#
    def calc_body_to_base_coordinates(self, q, body, coordinates_in_body_frame=np.zeros(3)):
        ...
        """
        # q = q.tolist()
        # coordinates_in_body_frame = coordinates_in_body_frame.tolist()
        # q = [1.1,0,2,2,1]
        # coordinates_in_body_frame = [1.1,0,0]
        # p = rbdl.CalcBodyToBaseCoordinates(self.model, q.tolist(), body.value, coordinates_in_body_frame.tolist(),
        ...
        # Q=[1.1,0,2,2]
        # body_a_id = 1
        # p = rbdl.CalcBodyToBaseCoordinates(self.model, Q, body_a_id, local_pos, update_kinematics)

        # update_kinematics = False
        # local_pos=[1.1,0,0]
        # Q=[1.1,0,2,2,1]
        # body_a_id = 5
        # p = rbdl.CalcBodyToBaseCoordinates(self.model, q, body.value, coordinates_in_body_frame, update_kinematics)
        p = rbdl.CalcBodyToBaseCoordinates(self.model, q, body.value, coordinates_in_body_frame, False)
        return p
#
    def calc_base_to_body_coordinates(self, q, body, coordinates_in_base_frame=np.zeros(3)):
        ...
        # Q=[1.1,0,2,2]
        # body_a_id = 1
        p = rbdl.CalcBaseToBodyCoordinates(self.model, q, body.value, coordinates_in_base_frame, False)
...
#
    def calc_point_Jacobian(self, q, body, coordinates_in_body_frame=np.zeros(3)):
        ...
        # Q=[1.1,0,2,2]
        # body_a_id = 1
        # J = np.zeros([3, 4]).tolist()
        # rbdl.CalcPointJacobian(self.model, Q, body_a_id, local_pos, J, update_kinematics)

        # J = np.zeros([3, self.qdot_size])
        J = rbdl.CalcPointJacobian(self.model, q, body.value, coordinates_in_body_frame, self.uk)
        return J
#
    def calc_point_acceleration(self, q, qdot, qddot, body, coordinates_in_body_frame=np.zeros(3)):
        ...
        """
        acc = rbdl.CalcPointAcceleration(self.model, q, qdot, qddot, body.value, coordinates_in_body_frame, self.uk)
        return acc
#
    def calc_point_velocity(self, q, qdot, body, coordinates_in_body_frame=np.zeros(3)):
        ...
        # Qdot=[1.1,0,2,2,1]
        # body_a_id = 1
        # vel = rbdl.CalcPointVelocity(self.model, Q, Qdot, body_a_id, local_pos, update_kinematics)
...
#
    def update_kinematics(self, q, qdot, qddot):
        ...
        :param qddot: Robot acceleration qddot in shape [dof].
        """
        a = rbdl.UpdateKinematics(self.model, q, qdot, qddot)
